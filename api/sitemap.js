...
module.exports = async (req, res) => {
  const staticPaths = ['/', '/for-business', '/alternative-to-fresha', '/alternative-to-timely', '/privacy-policy', '/terms-conditions'];
  let venueSlugs = [];
  ...
  const urls = [
    ...staticPaths.map(p => ({ loc: `${SITE_URL}${p}`, priority: p === '/' ? '1.0' : (p === '/for-business' ? '0.9' : (p.startsWith('/alternative-to-') ? '0.7' : '0.3')) })),
