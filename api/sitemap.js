...
module.exports = async (req, res) => {
  const staticPaths = ['/', '/explore', '/alternative-to-fresha', '/alternative-to-timely', '/privacy', '/terms'];
  let venueSlugs = [];
  ...
  const urls = [
    ...staticPaths.map(p => ({ loc: `${SITE_URL}${p}`, priority: p === '/' ? '1.0' : (p === '/explore' ? '0.9' : (p.startsWith('/alternative-to-') ? '0.7' : '0.3')) })),
